...
func RunBoringGame() {
	...
	tcp := flag.Bool("tcp", true, "Server will listen on TCP")
	...
	if *shutdownDelayMin > 0 && *shutdownDelaySec > 0 {
		log.Error().Msg("Cannot set both --automaticShutdownDelayMin and --automaticShutdownDelaySec")
	}
	if *readyIterations > 0 && *shutdownDelayMin <= 0 && *shutdownDelaySec <= 0 {
		log.Error().Msg("Must set a shutdown delay if using ready iterations")
	}

	log.Print("Creating SDK instance")
	s, err := sdk.NewSDK()
	if err != nil {
		log.Error().Msgf("Could not connect to sdk: %v", err)
		return
	}
	...
	if *passthrough {
		...
		gs, err = s.GameServer()
		if err != nil {
			log.Error().Msgf("Could not get gameserver port details: %s", err)
		}
...
func shutdownAfterNAllocations(s *sdk.SDK, readyIterations, shutdownDelaySec int) {
	gs, err := s.GameServer()
	if err != nil {
		log.Error().Msgf("Could not get game server: %v", err)
	}
	...
	if err := s.WatchGameServer(func(gs *coresdk.GameServer) {
		...
		if lastAllocated != la {
			log.Info().Msg("Game Server Allocated")
			lastAllocated = la
			remainingIterations--
			// Run asynchronously
			go func(iterations int) {
				...
				if iterations > 0 {
					log.Info().Msg("Moving Game Server back to Ready")
					readyErr := s.Ready()
					if readyErr != nil {
						log.Error().Msgf("Could not set game server to ready: %v", readyErr)
					}
					log.Info().Msg("Game Server is Ready")
					return
				}

				log.Info().Msg("Moving Game Server to Shutdown")
				if shutdownErr := s.Shutdown(); shutdownErr != nil {
					log.Error().Msgf("Could not shutdown game server: %v", shutdownErr)
				}
		...
		log.Error().Msgf("Could not watch Game Server events, %v", err)
	}
}
...
func udpListener(port *string, s *sdk.SDK, cancel context.CancelFunc) {
	...
	if err != nil {
		log.Error().Msgf("Could not start UDP server: %v", err)
	}
...
func udpRespond(conn net.PacketConn, sender net.Addr, txt string) {
	if _, err := conn.WriteTo([]byte(txt), sender); err != nil {
		log.Error().Msgf("Could not write to udp stream: %v", err)
	}
}
...
func tcpListener(port *string, s *sdk.SDK, cancel context.CancelFunc) {
	...
	if err != nil {
		log.Error().Msgf("Could not start TCP server: %v", err)
	}
...
func tcpRespond(conn net.Conn, txt string) {
	...
	if _, err := conn.Write([]byte(txt + "\n")); err != nil {
		log.Error().Msgf("Could not write to TCP stream: %v", err)
	}
}
...
func ready(s *sdk.SDK) {
	err := s.Ready()
	if err != nil {
		log.Error().Msgf("Could not send ready message")
	}
}
...
func allocate(s *sdk.SDK) {
	err := s.Allocate()
	if err != nil {
		log.Error().Msgf("could not allocate gameserver: %v", err)
	}
}
...
func reserve(s *sdk.SDK, duration time.Duration) {
	if err := s.Reserve(duration); err != nil {
		log.Error().Msgf("could not reserve gameserver: %v", err)
	}
}
...
func readPacket(conn net.PacketConn, b []byte) (net.Addr, string) {
	...
	if err != nil {
		log.Error().Msgf("Could not read from udp stream: %v", err)
	}
...
func gameServerName(s *sdk.SDK) string {
	...
	gs, err := s.GameServer()
	if err != nil {
		log.Error().Msgf("Could not retrieve GameServer: %v", err)
	}
	var j []byte
	j, err = json.Marshal(gs)
	if err != nil {
		log.Error().Msgf("error mashalling GameServer to JSON: %v", err)
	}
...
func setAnnotation(s *sdk.SDK, key, value string) {
	...
	if err != nil {
		log.Error().Msgf("could not set annotation: %v", err)
	}
}
...
func setLabel(s *sdk.SDK, key, value string) {
	...
	if err != nil {
		log.Error().Msgf("could not set label: %v", err)
	}
}
...
func setPlayerCapacity(s *sdk.SDK, capacity int64) {
	...
	if err := s.Alpha().SetPlayerCapacity(capacity); err != nil {
		log.Error().Msgf("could not set capacity: %v", err)
	}
}
...
func getPlayerCapacity(s *sdk.SDK) string {
	...
	if err != nil {
		log.Error().Msgf("could not get capacity: %v", err)
	}
...
func playerConnect(s *sdk.SDK, id string) {
	...
	if _, err := s.Alpha().PlayerConnect(id); err != nil {
		log.Error().Msgf("could not connect player: %v", err)
	}
}
...
func playerDisconnect(s *sdk.SDK, id string) {
	...
	if _, err := s.Alpha().PlayerDisconnect(id); err != nil {
		log.Error().Msgf("could not disconnect player: %v", err)
	}
}
...
func playerIsConnected(s *sdk.SDK, id string) string {
	...
	if err != nil {
		log.Error().Msgf("could not retrieve if player is connected: %v", err)
	}
...
func getConnectedPlayers(s *sdk.SDK) string {
	...
	if err != nil {
		log.Error().Msgf("could not retrieve connected players: %s", err)
	}
...
func getPlayerCount(s *sdk.SDK) string {
	...
	if err != nil {
		log.Error().Msgf("could not retrieve player count: %s", err)
	}
...
func doHealth(sdk *sdk.SDK, ctx context.Context) {
	...
	for {
		...
		err := sdk.Health()
		if err != nil {
			log.Error().Msgf("Could not send health ping, %v", err)
		}
		select {
		case <-ctx.Done():
